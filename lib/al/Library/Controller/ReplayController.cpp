...
// NON_MATCHING: regswap (https://decomp.me/scratch/sdwKp)
void ReplayController::calc(u32 prevHold, bool prevPointerOn) {
    ...
    if (isReplaying() && mPadDataReader) {
        ...
        mPadTrig = frameData.trig;
        mPadHold = frameData.hold;
        mLeftStick.set(frameData.leftStick);
        mPointer.set(frameData.pointer);
    ...
    if (isRecording() && mPadDataWriter) {
        // mismatch here
        mPadDataWriter->write({
            .trig = mPadTrig,
            .hold = mPadHold,
            .leftStick = mLeftStick,
            .pointer = mPointer,
        });
    }
}
