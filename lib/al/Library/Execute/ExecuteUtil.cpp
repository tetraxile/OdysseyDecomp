...
void addToExecutorMovement(al::LiveActor* actor) {
    actor->getExecuteInfo()->getRequestKeeper()->request(actor, al::ExecuteRequestKeeper::Request_Movement);
}

void addToExecutorDraw(al::LiveActor* actor) {
    actor->getExecuteInfo()->getRequestKeeper()->request(actor, al::ExecuteRequestKeeper::Request_Draw);
}

void removeFromExecutorMovement(al::LiveActor* actor) {
    actor->getExecuteInfo()->getRequestKeeper()->request(actor, al::ExecuteRequestKeeper::Request_RemoveFromMovement);
}

void removeFromExecutorDraw(al::LiveActor* actor) {
    actor->getExecuteInfo()->getRequestKeeper()->request(actor, al::ExecuteRequestKeeper::Request_RemoveFromDraw);
}

void updateExecutorDrawRec(al::LiveActor* baseActor, al::LiveActor* actor) {
    if (al::isAlive(actor) && actor->getExecuteInfo()) {
        al::ActorExecuteInfo* executeInfo = actor->getExecuteInfo();
        for (s32 i = 0; i < executeInfo->getDrawerCount(); i++) {
            executeInfo->getDrawer(i)->updateModel(actor->getModelKeeper()->getModelCtrl());
        }
    }
